...
class Inductor():
    ...
    def DDP(self, geometry):
        ...
        return self.DDP
    def __str__(self):
        s = "\n" + "Длина индуктора:" + str(self.LCA) + ",м"
        s += "\n" + "Величина зазора между индуктором и заготовкой:" + str(self.ZCP) + ",м"
        s += "\n" + "Диаметр индуктора:" + str(self.DCA) + ",м"
        s += "\n" + "Глубина проникновения ИМП в материал заготовки: " + str(self.BP) + " ,м"
        s += "\n" + "Глубина проникновения ИМП в материал индуктор: " + str(self.BC) + " ,м"
        s += "\n" + "Паразитная индуктивность разрядного контура: " + str(self.LDC) + " ,Гн"
        s += "\n" + "Частота разряда при наличии только паразитных индуктивностей: " + str(self.FDC) + " ,Гц"
        s += "\n" + "Коэффициент К1: " + str(self.K1)
        s += "\n" + "Коэффициент К2: " + str(self.K2)
        s += "\n" + "Коэффициент К3: " + str(self.K3)
        s += "\n" + "Коэффициент К4: " + str(self.K4)
        s += "\n" + "Значение эквивалентного зазора между индуктором и заготовкой: " + str(self.ZEK)
        s += "\n" + "Количество витков индуктора: " + str(self.NCTC)
        s += "\n" + "Целое количество витков индуктора: " + str(self.NCW)
        s += "\n" + "Расчетное количество рабочих витков" + str(self.NCWC)
        s += "\n" + "Количество свободных витков" + str(self.NCF)
        s += "\n" + "Расчетный шаг витков индуктора: " + str(self.SCIC)
        s += "\n" + "Необходимая энергия разряда МИУ: " + str(self.WR) + ",Дж"
        s += "\n" + "Суммарная индуктивность: " + str(self.LUC) + " ,Гн"
        s += "\n" + "Давление  " + str(self.PM) + "Па"
        s += "\n" + "Скорость  " + str(self.VCR) + "м\с"
        return s
