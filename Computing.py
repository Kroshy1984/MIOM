...
class Form():
    ...
    def __str__(self):
        F = f"Внутренний диаметр трубчатой заготовки:{self.DIB},м"
        F += f"\nВнутренний радиус трубчатой заготовки:{self.RIB} ,м"
        F += f"\nCредняя величина деформации заготовки:{self.EPS} ,м"
        F += f"\nДинамическое значение коэффициента аппроксимации кривой упрочнения:{self.BCMD}"
        F += f"\nУдельная работа деформации:{self.WYD},Дж"
        F += f"\nДеформируемый объем заготовки:{self.DVB},mm3"
        F += f"\nРабота деформации заготовки:{self.WDB} ,Дж"
        F += "\nНеобходимая энергия для выполнения операции:{self.WMIR},Дж"
        F += f"\nЭнергоемкость установки:{self.WMUR},Дж"
        return F