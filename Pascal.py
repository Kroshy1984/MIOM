...
class Pascal():
    def __init__(self):
        ...
        hb = 10e-3
        lv = 270e-3
        dv = 153e-3  # Dи_в ВНУТРЕННИЙ ДИАМЕТР ИНДУКТОРА , м #вводится или рассчитывается
        self.n1 = 7  # self.n1_и ЧИСЛО ВИТКОВ ИНДУКТОРА
        self.l1 = 0.036  # self.l1_и ДЛИНА (ВЫСОТА) ИНДУКТОРА, м
        R0 = 24.0e-3  # * math.pow(10, -3) # Ro_у СОПРОТИВЛЕНИЕ УСТАНОВКИ, Ом # вводится из таблицы материалов
    ...
    def var1(self):
        ...
        if (ss7-ss6 > 0):
            ...
            print(
                f"Time_tek - {int(self.Time_tek * pow(10, 6))},U_tek- {self.U_tek},Iind- {self.Iind}, - {self.Izag},P_tek - {self.P_tek}, {self.y[1] * 100},.y[2]- {self.y[2] * 1000}, S_tek - {self.S_tek},V_tek- {self.V_tek}")
    ...
    def var2(self):
            ...
            if (self.x3 == h_tek / 3.0): #TODO:(x3 <= h_tek/3.0)
                xx = self.x3 / 2.0
            else:
                xx = h_tek / 3.0
            if (self.x3 == h_tek):
